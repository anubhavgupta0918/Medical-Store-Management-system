...
columns = ('Sl No', 'Name', 'Type', 'Quantity Left', 'Cost', 'Purpose', 'Expiry Date', 'Rack location', 'Manufacture')
                                 ...
                                 #  Admin main module  22-54
def open_win():
    global apt, flag
    flag = 'apt'
    apt = Tk()
    apt.title("MEDIHUB")
    Label(apt, text="MEDIHUB", font='Helvetica 19 bold').grid(row=0, column=0, columnspan=3)
    Label(apt, text='_' * 80).grid(row=1, column=0, columnspan=3)
    #Label(apt, text='_' * 80).grid(row=3, column=0, columnspan=3)

    Label(apt, text="Stock Maintenance", font='Helvetica 10 bold', bg='skyblue', fg='black').grid(row=2, column=0, padx=10,
                                                                                         pady=10)
    Button(apt, text='New V.C.', width=21, bg='grey32', fg='white', command=val_cus).grid(row=4, column=0, padx=5,
                                                                                         pady=5)
    Button(apt, text='Add product to Stock', bg='grey32', fg='white', width=21, command=stock).grid(row=5, column=0,
                                                                                                   padx=5, pady=5)
    Button(apt, text='Delete product from Stock', bg='grey32', fg='white', width=21, command=delete_stock).grid(row=6,
                                                                                                             column=0,
                                                                                                             padx=5,
                                                                                                             pady=5)

    Label(apt, text="Access Database", font='Helvetica 10 bold', bg='skyblue', fg='black').grid(row=2, column=1)
    Button(apt, text='Modify', width=15, bg='grey32', fg='white', command=modify).grid(row=4, column=1, padx=10, pady=10)
    Button(apt, text='Search', width=15, bg='grey32', fg='white', command=search).grid(row=5, column=1, padx=10, pady=10)
    Button(apt, text='Expiry Check', bg='grey32', fg='white', width=15, command=exp_date).grid(row=6, column=1, padx=10,
                                                                                            pady=10)

    Label(apt, text="Handle Cash Flows", font='Helvetica 10 bold', bg='skyblue', fg='black').grid(row=2, column=2)
    Button(apt, text="Check Today's Revenue", bg='grey32', fg='white', width=21, command=show_rev).grid(row=5,
                                                                                                         column=2)
    Button(apt, text='Billing', width=21, bg='grey32', fg='white', command=billing).grid(row=4, column=2, padx=10,
                                                                                          pady=10)
    Button(apt, text='Logout', bg='red', fg='white', width=21, command=again).grid(row=6, column=2, padx=10, pady=10)

    #Button(apt, text='Add QR code', bg='skyblue', fg='black', width=21, command=add_qr).grid(row=7, column=0, padx=10, pady=10)
    Button(apt, text='Show QR code', bg='skyblue', fg='black', width=21, command=sh_qr).grid(row=7, column=1, padx=10, pady=10)
    Button(apt, text='QR code generator', bg='skyblue', fg='black', width=21, command=qr_gen).grid(row=7, column=2, padx=10,
                                                                                             pady=10)
    apt.mainloop()
...
##############################################
def sh_qr():
    global val, flag, dbt, Subject, cur, c, name,sqr
    apt.destroy()

    flag = 'sqr'
    sqr = Tk()
    # apt.title("QR Code Generator")
    sqr.title("QR Code Viewer")

    button_quit = Button(sqr, text='Main Menu', command=main_menu, bg='green', fg='white')
    button_quit.pack()
    e = Entry(sqr, width=40, bg='white', borderwidth=3)
    e.pack()
    e.insert(0, '.png')
    my_label = Label()
    my_label.pack()
    def myClick():
        if len(e.get()) != 0:
            link = r'C:/Users/ANUBHAV/Desktop/cse/test_medihub/' + e.get()
    ...
    myButton = Button(sqr, text='Search', command=myClick,
                      bg='red', fg='white')
    myButton.pack()


    #button = Button(sqr, text="Main Menu", width=15, bg='green', fg='white', command=main_menu)
...
def qr_gen():
    global val, flag, dbt, Subject, cur, c, name,qr
    apt.destroy()
    ...
    def generate():
        if len(Subject.get()) != 0:
            ...
            messagebox.showinfo("Please Enter some medicine")
        try:
            showcode()
        except:
            pass
    ...
    def showcode():
        ...
        subLabel.config(text="QR of " + name.get())




   # Label(qr, bg='blue', fg='white', text="****GENERATE QR CODE****").grid(row=0, column=1, sticky=N + S + W + E)
    Sub = Label(qr, text="Enter Address")
    Sub.grid(row=0, column=0, sticky=N + S + W + E)

    FName = Label(qr, text="Enter Medicine name")
    FName.grid(row=1, column=0, sticky=N + S + W + E)

    Subject = StringVar()
    SubEntry = Entry(qr, textvariable=Subject)
    SubEntry.grid(row=0, column=1, sticky=N + S + W + E)

    name = StringVar()
    nameEntry = Entry(qr, textvariable=name)
    nameEntry.grid(row=1, column=1, sticky=N + S + W + E)

    button = Button(qr, text="Generate", width=15, command=generate)
    button.grid(row=0, column=3, sticky=N + S + W + E)

    imageLabel = Label(qr)
    imageLabel.grid(row=2, column=1, sticky=N + S + W + E)

    subLabel = Label(qr, text="")
    subLabel.grid(row=3, column=1, sticky=N + S + W + E)

    saveB = Button(qr, text="Save as PNG", width=15, command=save)
    ...
    for row in range(Rows + 1):
        qr.grid_rowconfigure(row, weight=2)

    for col in range(Columns + 1):
        qr.grid_columnconfigure(col, weight=1)
    button = Button(qr, text="Main Menu", width=15, bg='green', fg='white', command=main_menu)
    button.grid(row=2, column=3,padx=10,pady=10 )
    #qr.mainloop()
    #button= Button(d, width=20, text='Main Menu', command=main_menu)\
    #button.grid(row=2, column=3,padx=10,pady=10)
    qr.mainloop()
...
def delete_stock():   #delete product
...
def stock():
    ...
    Button(sto, width=15, text='Submit', bg='sea green', fg='white', command=submit).grid(row=12, column=1)
    ...
    Button(sto, width=10, text='Main Menu', bg='blue', fg='white', command=main_menu).grid(row=12, column=5)
    ref()
    sto.mainloop()
...
def billing():    #Billing
    ...
    Button(st, text='Add to bill', bg='sea green', fg='white', width=15, command=append2bill).grid(row=8, column=6)
    Label(st, text='QUANTITY', width=20, relief='ridge').grid(row=7, column=5)
    qtys = Entry(st)
    qtys.grid(row=8, column=5)
    refresh()
    Button(st, width=15, text='Main Menu', bg='blue', fg='white', command=main_menu).grid(row=1, column=6)
    ...
    Button(st, width=15, text='Save Bill', bg='sea green', fg='white', command=make_bill).grid(row=7, column=6)
...
def make_bill():
    ...
    m += "===============================================\n"
    m += "                                  No :%s\n\n" % det[5]
    m += " MEDIHUB MEDICAL STORE COMPANY\n"
    m += " DELHI\n\n"
    m += "-----------------------------------------------\n"
    if t == 1:
        m += "Name: %s\n" % named
...
def val_cus():    #add customer  724-744
    ...
    Button(val, text='Submit', bg='sea green', fg='white', command=val_get).grid(row=5, column=1)
    Button(val, text='Main Menu', bg='blue', fg='white', command=main_menu).grid(row=5, column=2)
...
def again():
    ...
    root.title('MEDIHUB')
    Label(root, text='MEDIHUB MEDICAL STORE COMPANY',font='Helvetica 11 bold').grid(row=0, column=0, columnspan=5)
    Label(root, text="M.K road jalandhar,punjab(India)").grid(row=1, column=0, columnspan=5)
    ...
    Button(root, width=6, bg='sea green', fg='white', text='Enter', command=check).grid(row=5, column=1)
    Button(root, width=6, bg='red', fg='white', text='Close', command=root.destroy).grid(row=5, column=2)
    root.mainloop()
...
def open_cus():
    ...
    apt = Tk()
    apt.title("Interface")
    Label(apt, text="*** MEDIHUB MEDICAL DRUG STORE ***", bg='blue', fg='white').grid(row=0, column=0)
